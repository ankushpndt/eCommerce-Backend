...
const privateRoute = require("../middleware/verifyToken");
...
router
  ...
  .delete(privateRoute, async (req, res) => {
    ...
    try {
      ...
      res.status(400).json({
        success: false,
        message: "Product was not deleted from the cart",
        errorMessage: error.message,
      });
    }
  });

router.post("/:productId/:quantity", privateRoute, async (req, res) => {
  try {
    const productId = req.product._id;
    const userId = req.user._id;
    const newQuantity = Number(req.params.quantity);
    const isProductInCart = await isProductInCartFun(userId, productId);
